...
/**
 * Goal satisfied by exactly one navigation position.
 */
public record ExactPathGoal(PathPosition representative) implements PathGoal {
    public ExactPathGoal {
        Objects.requireNonNull(representative, "representative");
    }

    @Override
    public boolean isSatisfiedBy(PathPosition position) {
        return representative.equals(position);
    }

    @Override
    public double estimate(PathPosition position, PathPosition start, HeuristicWeights weights, IHeuristicStrategy strategy) {
        return strategy.calculate(new HeuristicContext(position, start, representative, weights));
    }
}
